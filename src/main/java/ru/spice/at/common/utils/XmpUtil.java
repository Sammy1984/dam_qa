...
public class XmpUtil {
    ...
    /**
     * Получаем XMP метадату из файла
     *
     * @param bytes  изображение в виде байтов
     * @return XMP метадата
     */
    @SneakyThrows
    public static XMPMeta getXMPFromBytes(byte[] bytes) {
        Metadata metadata = ImageMetadataReader.readMetadata(new ByteArrayInputStream(bytes));
        XmpDirectory xmpDirectory = metadata.getFirstDirectoryOfType(XmpDirectory.class);
        return xmpDirectory.getXMPMeta();
    }
    ...
    public static XMPMeta extractXMPMeta(String filename) {
        if (!filename.toLowerCase().endsWith(".jpg")
            ...
            log.info("XMP parse: only jpeg file is supported");
            return null;
        }
        try {
    ...
    public static boolean writeXMPMeta(String filename, XMPMeta meta) {
        ...
        try {
            ...
            log.info("Write file failed:" + filename);
            e.printStackTrace();
            return false;
        } finally {
            if (os != null) {
    ...
    public static boolean writeXMPMeta(InputStream inputStream, OutputStream outputStream,
        ...
        try {
            ...
            log.info("Write to stream failed");
            e.printStackTrace();
            return false;
        } finally {
